...
<div class=" position-fixed h-100 d-flex justify-content-end align-items-center" style=" top:0; right: 5px; z-index:4">
    <div class="pull-right">
        <a class="fa-stack h4 text-light" href="{{ route('create_post') }}" data-toggle="tooltip" data-placement="left"
            title="New post">
        ...
        <article class="card mb-5 border-0">
            <section class=" card-header pb-1 px-3">
                <div class="d-flex justify-content-between align-items-start mb-2">
                    <p class="h5 heading m-0">{{$excerpt->title}}</p>
                    <p class="category badge p-2 m-0 gc-p">{{$excerpt->category->category}}</p>
                </div>
                <div class="d-flex">
                    <img src="{{ asset('/images/body.jpg') }}" class="rounded-circle" height="40" width="40">
                    <div class="d-inline-block pl-2 w-100 author">
                        <p class="m-0 border-light border-bottom">{{$excerpt->user->name}}</p>
                        <p class="m-0">{{ date("D jS F Y ", strtotime($excerpt->created_at))}}</p>
                    </div>
                </div>
            </section>
            ...
            <section class="card-footer d-flex justify-content-between px-3">
                ...
                <a href="{{ route('article', ['title'=> str_replace(' ', '-', $excerpt->title) ]) }}"
        ...
        <div class=" card mb-3">
            ...
            <div class="list-group">
                ...
                <a href="{{ route('article', ['title'=> str_replace(' ', '-', $rp->title) ]) }}"
                    ...
                    <p class="m-0">
                        <span
                            class="fa fa-calendar-check-o mr-2"></span>{{ date("D jS M Y ", strtotime($rp->created_at))}}
                    </p>
                </a>
                @empty
